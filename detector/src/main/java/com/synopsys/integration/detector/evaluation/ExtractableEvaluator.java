...
import com.synopsys.integration.detector.result.ExceptionDetectorResult;

public class ExtractableEvaluator {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());

    private final DetectorEventListener detectorEventListener;

    public ExtractableEvaluator(final DetectorEventListener detectorEventListener) {
        this.detectorEventListener = detectorEventListener;
    }

    private void prepare(final DetectorEvaluation result) {
        if (result.isApplicable()) {
            detectorEventListener.extractableStarted(result.getDetector());
            try {
            ...
            detectorEventListener.extractableEnded(result.getDetector());
        }
    }
