...
public class ExtractionEvaluator {
    ...
    public ExtractionResult performExtractions(final List<DetectorEvaluation> results) {
        ...
        for (int i = 0; i < extractable.size(); i++) {
            ...
            ExtractionEnvironment extractionEnvironment = new ExtractionEnvironment(new File(""));//TODO: REal file here
        ...
        final List<CodeLocation> codeLocations = extractable.stream()
                                                           .filter(it -> it.wasExtractionSuccessful())
...
//        logger.info(ReportConstants.SEPERATOR);
        ...
        try {
            Detector detector = result.getDetector();
            Extraction extraction = detector.extract(result.getExtractionEnvironment());
...
//        logger.info(ReportConstants.SEPERATOR);
        ...
        if (result.getExtraction().result == Extraction.ExtractionResultType.EXCEPTION) {
            logger.error("Exception:", result.getExtraction().error);
        } else if (result.getExtraction().result == Extraction.ExtractionResultType.FAILURE) {
