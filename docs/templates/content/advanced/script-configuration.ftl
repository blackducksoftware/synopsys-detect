...
| DETECT_LATEST_RELEASE_VERSION | Run a given ${solution_name} version |  A Detect version (example: 5.6.2) | If you would like to run a ${solution_name} version other than the latest, set DETECT_LATEST_RELEASE_VERSION to the ${solution_name} version you would like to run (for example: 5.6.2). DETECT_SOURCE has precedence over DETECT_LATEST_RELEASE_VERSION. You can see the available ${solution_name} versions in [Artifactory](${binary_repo_url_base}/webapp/#/artifacts/browse/tree/General/${binary_repo_repo}/${binary_repo_pkg_path}/${project_name}). |
