...
1. For more troubleshooting information: Run ${solution_name} with `-d`. This will generate a diagnostic zip that contains many useful intermediate files and logs, including the generated BDIO (.jsonld) files and signature scanner logs.
...
1. Remember to consider the possibility that the ${blackduck_product_name} user lacks the necessary permissions (to create the project, update the BOM, receive notifications, etc.) in ${blackduck_product_name}. For more information, see [Black Duck user role requirements](/advanced/BD-user-role-requirements).
...
1. For issues related to tools invoked by ${solution_name} (${blackduck_product_name} signature scanner, Docker Inspector, etc.), please check that tool's documentation.
1. For issues related to incorrect components in the ${blackduck_product_name} BOM: ${solution_name} has a great deal of control over matches produced by Detectors (that are written to BDIO/.jsonld files), but no control over matches produced by the signature scanner. When investigating an incorrect component in a ${blackduck_product_name} BOM, you need to determine whether the component was contributed by a Detector, or by the signature scanner: On the ${blackduck_product_name} Components tab for the project/version: Click on the "N Matches" link next to the component. The next screen lists the matches on the right side. Matches from the signature scanner have a filename in the Name column. Matches from Detectors have an external ID (such as "org.hamcrest:hamcrest-core:1.3") in the Name column.
1. For issues related to components missing from or or incorrectly categorized in the ${blackduck_product_name} BOM: ${solution_name} has a great deal of control over the production of .jsonld files (use -d to save these), but no control over how they are converted into a BOM by ${blackduck_product_name}. A good first step is to determine whether the .jsonld files produced are correct. If they are incorrect, the problem is related to what ${solution_name} is doing. If they are correct, but the BOM is incorrect, the problem is related to what ${blackduck_product_name} is doing. Similarly, ${solution_name} is responsible for passing the correct arguments to the signature scanner, but has little control over the results it produces.
...
-d: When enabled, diagnostic mode collects all files generated by Synopsys Detect and zips the files using a unique run ID. It includes logs, BDIO files, extraction files, and reports.
-de --diagnosticExtended: When diagnosticExtended is enabled, Synopsys Detect collects relevant files such as lock files and build artifacts.
The zip file is created inside the Synopsys Detect output directory, and must be provided when support issues are opened.

Synopsys does not recommend keeping this option on, as it does not clean up the zip file(s); regardless of the Detect cleanup flag.

Diagnostic mode:

* Sets the output log level to DEBUG.
* Zips the entire run directory of Detect which includes:
* All BDIO files created by Detect.
* Any intermediary files generated by Detect, such as Gradle inspector output files.
* Includes any additional reports Detect might make such as dependency counts.
* Additionally -de or —diagnosticExtended includes additional relevant files such as:
* Npm package locks.
* vendor.config.
* C/C++ compilation database.

The option -de allows the collection of relevant files not generated by Detect; for example, lock files. By default, diagnostic mode only include files generated by Detect where -de might include files it finds.  For example, a package.lock that is too deep for Detect’s search depth is not included, or a package.lock that was not parsed due to exclusion.
