...
For example, if you build your application on ubuntu:latest (your Dockerfile starts with FROM ubuntu:latest), you can exclude components from the ubuntu layer(s) so that the components generated by ${solution_name} (via Docker Inspector and the ${blackduck_signature_scanner_name}) contain only components from your application layers.
...
./detect.sh ... --detect.docker.image=<your application image> --detect.docker.passthrough.docker.platform.top.layer.id=sha256:b079b3fa8d1b4b30a71a6e81763ed3da1327abaf0680ed3ed9f00ad1d5de5e7c
    ...
    In this mode, there may be some loss in match accuracy from the ${blackduck_signature_scanner_name} because, in this scenario, the ${blackduck_signature_scanner_name} may be deprived of some contextual information (such as the operating system files that would enable it to determine the Linux distro), and that that may negatively affect it's ability to accurately identify components.
