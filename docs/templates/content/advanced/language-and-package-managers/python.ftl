...
* If the packages have been installed into a virtual environment for your project, you will need to run ${solution_name} from within that virtual environment.
* If you are using a tool (such as poetry) that sets up a python virtual environment, you will need to run ${solution_name} from within that virtual environment.

Recommendations:

* Be sure that ${solution_name} is finding the correct python executable (run with logging level at DEBUG and read the log). This is a particular concern if your system has multiple versions of python installed; you want to be sure ${solution_name} is using the right python version.
...
* If there are any dependencies specified in requirements.txt that are not specified in setup.py, provide the requirements.txt file via the ${solution_name} property.
* If you are using a virtual environment, make sure to switch to that virtual environment when you run ${solution_name}. This also applies when you are using a tool (such as poetry) that sets up a python virtual environment.
...
To troubleshooting of the Pipenv detector, start by running `pipenv graph --bare`, and making sure that output looks correct (since this is the basis from which ${solution_name} constructs the BDIO). If the output of `pipenv graph --bare` does not look correct, make sure the packages (dependencies) have been installed into the pipenv virtual environment (`pipenv install`).
