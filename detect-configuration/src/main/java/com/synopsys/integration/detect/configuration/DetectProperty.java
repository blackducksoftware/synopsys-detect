...
public enum DetectProperty {
    ...
    @HelpDescription("This disables any Black Duck communication.  If true, Detect does not upload BDIO files, it does not check policies, and it does not download and install the signature scanner.")
    ...
    @HelpGroup(primary = GROUP_PROXY, additional = { SEARCH_GROUP_BLACKDUCK, DEFAULT_HELP })
    @HelpDescription("Proxy port number.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The output directory for all BDIO files. If not set, the BDIO files are placed in a 'bdio' subdirectory of the output path.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { GROUP_SOURCE_SCAN })
    @HelpDescription("Enables you to adjust the depth to which Detect searches when creating signature scanner exclusion patterns.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If this URL is set, an attempt is made to use it to download the signature scanner. The server URL must respect the Black Duck URLs for different operating systems.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("To use a local signature scanner, specify the path where the signature scanner was unzipped. This resembles 'scan.cli-x.y.z' and includes the 'bin, icon, jre, and lib' directories of the expanded scan.cli.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("To use a local signature scanner and force offline, specify the path where the signature scanner was unzipped. This resembles 'scan.cli-x.y.z' and includes the 'bin, icon, jre, and lib' directories of the expanded scan.cli.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The number of scans to run in parallel.  The default value is 1, but if you specify -1, the number of processors on the machine are used.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("Specifies that only these paths are scanned.")
    ...
    @HelpDescription("Use this value to enable the various snippet scanning modes. For a full explanation, refer to the Black Duck signature scanner documentation.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_GLOBAL, GROUP_SOURCE_SCAN })
    @HelpDescription("If set to true, the signature scanner, if supported by your Black Duck version, upload source code to Black Duck.")
    ...
    @HelpDescription("If set, this aggregates all the BOMs to create a single BDIO file using the provided name.")
    ...
    @HelpDescription("If set to true, only detectors capable of running without a build are run.")
    ...
    @HelpDescription("If true, the files created by Detect are cleaned up.")
    ...
    @HelpDescription("The name of the project version from which to clone this project version. Respects the clone categories as set on the Black Duck server.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("An override for the name Detect uses for the scan file it creates. If supplied and multiple scans are found, Detect appends an index to each scan name.")
    ...
    @HelpDescription("The scheme to use when the package managers cannot determine a version, either 'text' or 'timestamp.'")
    ...
    @HelpDescription("The depth from source paths to search for files to determine if a detector applies.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { GROUP_DETECTOR, SEARCH_GROUP_GLOBAL, GROUP_SOURCE_SCAN })
    @HelpDescription("If true, the BOM tool search continues to look for nested BOM tools of the same type to the maximum search depth.  Refer to Detect help for more information.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { GROUP_DETECTOR, SEARCH_GROUP_GLOBAL, GROUP_SOURCE_SCAN })
    @HelpDescription("A comma-separated list of excluded directory names in the detector search.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { GROUP_DETECTOR, SEARCH_GROUP_GLOBAL, GROUP_SOURCE_SCAN })
    @HelpDescription("A comma-separated list of excluded directory name pattern in the detector search.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { GROUP_DETECTOR, SEARCH_GROUP_GLOBAL, GROUP_SOURCE_SCAN })
    @HelpDescription("If true, the BOM tool search excludes the default directory names. Refer to Detect help for more information.")
    ...
    @HelpDescription("If true, Detect ignores any products to which it cannot connect.")
    @HelpDetailed("If true, when Detect attempts to boot a product, it checks if it can communicate with it.  If not, Detect does not run the product.")
    ...
    @HelpGroup(primary = GROUP_DOCKER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("Version of the Docker Inspector to use. By default, Detect attempts to automatically determine the version to use.")
    ...
    @HelpGroup(primary = GROUP_DOCKER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If set to true, Detect attempts to run the Docker Inspector only if it finds a Docker client executable.")
    ...
    @HelpDescription("The path of the .net executable.")
    ...
    @HelpDescription("By default, all tools are included. To exclude specific detectors, specify the ones to exclude here. If you want to exclude all detectors, specify \"ALL\". Exclusion rules always win.")
    ...
    @HelpDescription("If true, Detect always exits with code 0.")
    ...
    @HelpDescription("If set to true, Detect attempts to run 'init' and 'ensure,' which can modify your development environment.")
    ...
    @HelpDescription("The Gradle build command.")
    ...
    @HelpGroup(primary = GROUP_GRADLE, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The override version of the Gradle Inspector to use. By default, Detect automatically tries to determine the correct Gradle version.")
    ...
    @HelpDescription("The path of the Gradle executable.")
    ...
    @HelpGroup(primary = GROUP_DETECTOR, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("By default, all tools are included. If you want to include specific tools, specify the ones to include here. Exclusion rules always win.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The path of the Java executable.")
    ...
    @HelpDescription("The Maven build command.")
    ...
    @HelpGroup(primary = GROUP_MAVEN, additional = { GROUP_SOURCE_SCAN })
    @HelpDescription("The names of the modules to exclude.")
    ...
    @HelpGroup(primary = GROUP_MAVEN, additional = { GROUP_SOURCE_SCAN })
    @HelpDescription("The names of the modules to include.")
    ...
    @HelpDescription("When set to true, a Black Duck notices report in text form is created in your source directory.")
    ...
    @HelpDescription("The output directory for the notices report. Default is the source directory.")
    ...
    @HelpDescription("Set this value to false to exclude your dev dependencies when run.")
    ...
    @HelpDescription("The path of the node executable used by npm.")
    ...
    @HelpGroup(primary = GROUP_NUGET, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If true, errors are logged and then ignored.")
    ...
    @HelpGroup(primary = GROUP_NUGET, additional = { GROUP_SOURCE_SCAN })
    @HelpDescription("The names of the projects in a solution to include.  Note that this overrides exclude.")
    ...
    @HelpGroup(primary = GROUP_NUGET, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The version of the NuGet Inspector. By default, Detect communicates with Artifactory.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The output path.")
    ...
    @HelpDescription("Set this value to false to exclude your dev requirements dependencies when ran.")
    ...
    @HelpDescription("Set to true to include only required packages.")
    ...
    @HelpDescription("The name of your PIP project. Use if your project's name cannot be correctly inferred from its setup.py file.")
    ...
    @HelpDescription("The version of your PIP project.  Use if your project's version name cannot be correctly inferred from its setup.py file")
    ...
    @HelpDescription("A comma-separated list of policy violation severities that fail Detect. If this is not set, Detect will not fail due to policy violations. A value of ALL is equivalent to all of the other possible values except UNSPECIFIED.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("An override for the project clone categories that are used when cloning a version. If the project already exists, use --detect.project.version.update to make sure these categories are set.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("If the project description is specified, your project version is created with this description.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The detector to choose when multiple detector types are found, and one must be chosen for project name and version. This property should be used with the detect.project.tool.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("An override for the name to use for the Black Duck project. If not supplied, Detect uses the tools to determine a reasonable project name. If that fails, the final part of the directory path where the inspection is taking place is used.")
    DETECT_PROJECT_NAME("detect.project.name", "Project Name", "3.0.0", PropertyType.STRING, PropertyAuthority.None),

    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("If a Black Duck project tier is specified, your project is created with this tier.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The tool priority for project name and version. The first tool in this list that provides a project name and version is used.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("An override for the version to use for the Black Duck project. If not supplied, Detect uses the tools to determine a reasonable version name. If that fails, the current date is used.")
    DETECT_PROJECT_VERSION_NAME("detect.project.version.name", "Version Name", "3.0.0", PropertyType.STRING, PropertyAuthority.None),

    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("If a project version nickname is specified, your project version is created with this nickname.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("If project version notes are specified, your project version is created with these notes.")
    ...
    @HelpGroup(primary = GROUP_PROJECT, additional = { SEARCH_GROUP_PROJECT_SETTING })
    @HelpDescription("If set to true, updates the project version with the configured properties. Refer to Detect help for more information.")
    ...
    @HelpGroup(primary = GROUP_PYTHON, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If true, Detect uses Python 3.0 if available on the class path.")
    ...
    @HelpDescription("The amount of time in seconds Detect waits for scans to finish and to generate reports.  For example, risk and policy check reports. When changing this value, remember that checking policies might have to wait for scans to process which may cause delays.")
    DETECT_REPORT_TIMEOUT("detect.report.timeout", "Report Generation Timeout", "5.2.0", PropertyType.LONG, PropertyAuthority.None, "300"),

    @HelpGroup(primary = GROUP_DETECTOR, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If set, Detect fails if it does not find the detector types supplied with this property.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If set to false, Detect does not automatically resolve the '~/' prefix in a Mac or Linux path to the user's home directory.")
    ...
    @HelpGroup(primary = GROUP_REPORT, additional = { SEARCH_GROUP_GLOBAL, SEARCH_GROUP_REPORT_SETTING })
    @HelpDescription("When set to true, a Black Duck risk report in PDF form is created.")
    ...
    @HelpGroup(primary = GROUP_REPORT, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The output directory for a risk report in PDF. The default value is the source directory.")
    ...
    @HelpDescription("If set to false, runtime dependencies are not included when parsing *.gemspec files.")
    ...
    @HelpGroup(primary = GROUP_RUBY)
    @HelpDescription("If set to true, development dependencies are included when parsing *.gemspec files.")
    ...
    @HelpGroup(primary = GROUP_SBT, additional = { GROUP_SOURCE_SCAN })
    @HelpDescription("The depth at which the SBT detector searches for report files.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The output directory for all scan files. If not set, the scan files are placed in a 'scan' subdirectory of the output path.")
    ...
    @HelpDescription("The source path to inspect.")
    DETECT_SOURCE_PATH("detect.source.path", "Source Path", "3.0.0", PropertyType.STRING, PropertyAuthority.DirectoryManager),

    @HelpGroup(primary = GROUP_BLACKDUCK_SERVER, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("Tests the connection to Black Duck using the current configuration.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("A comma-separated list of tools Detect allows. Included and not excluded tools are allowed to run if all criteria of the tool is met. Exclusion rules always win.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("A comma-separated list of tools Detect does not allow.  Excluded tools are not run even if all criteria for the tool is met. Exclusion rules always win.")
    ...
    @HelpDescription("Set this to true to scan production dependencies.")
    ...
    @HelpGroup(primary = GROUP_GENERAL, additional =  { SEARCH_GROUP_GLOBAL })
    @HelpDescription("If set to true, Detect waits for Synopsys products until results are available or the blackduck.timeout value is exceeded.")
    ...
    @HelpDescription("Timeout for response from Black Duck regarding your project reports. When changing this value, remember that checking policies might have to wait for a new scan to process which may cause delays.")
    ...
    @HelpDescription("The URL of the Black Duck server.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("Time in seconds to wait for rest connections to complete.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("The Black Duck username.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("The Black Duck password.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("The Black Duck API token.")
    ...
    @HelpDescription("The Black Duck proxy host.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB, GROUP_PROXY })
    @HelpDescription("The Black Duck proxy port.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB, GROUP_PROXY })
    @HelpDescription("The Black Duck proxy user name.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB, GROUP_PROXY })
    @HelpDescription("The Black Duck proxy password.)
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB, GROUP_PROXY })
    @HelpDescription("The NTLM proxy domain.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB, GROUP_PROXY })
    @HelpDescription("The NTLM proxy workstation.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("If true, automatically trusts the certificate for only the current run of Detect.")
    ...
    @HelpDescription("This disables Black Duck communication. If true, Detect does not upload BDIO files, does not check policies, and does not download and install the signature scanner.")
    ...
    @HelpGroup(primary = GROUP_HUB_CONFIGURATION, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("If true, during initialization Detect checks for Black Duck connectivity, and exits with a status code 0 if it cannot connect.")
    DETECT_DISABLE_WITHOUT_HUB("detect.disable.without.hub", "", "4.0.0", PropertyType.BOOLEAN, PropertyAuthority.None, "false"),

    @Deprecated
    @DetectDeprecation(description = "This property is changing. Please use --detect.ignore.connection.failures in the future.", failInVersion = DetectMajorVersion.SIX, removeInVersion = DetectMajorVersion.SEVEN)
    @HelpGroup(primary = GROUP_BLACKDUCK_SERVER, additional = { SEARCH_GROUP_BLACKDUCK, DEFAULT_HELP })
    @HelpDescription("If true, during initialization Detect checks for Black Duck connectivity and exits with status code 0 if it cannot connect.")
    ...
    @HelpDescription("If true, the default behavior of printing your configuration properties at startup is suppressed.")
    ...
    @HelpGroup(primary = GROUP_LOGGING)
    @HelpDescription("If true, the default behavior of printing the Detect results is suppressed.")
    ...
    @HelpDescription("By default, all tools are included. To exclude specific detectors, specify the ones to exclude here. To exclude all tools, specify \"ALL\". Exclusion rules always win.")
    ...
    @HelpDescription("By default, all tools are included. To include specific tools, specify the ones to include here. Exclusion rules always win.")
    ...
    @HelpDescription("The depth from source paths to search for SBT report files.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { GROUP_DETECTOR })
    @HelpDescription("The detector to choose when multiple detector types are found and one must be chosen for project name and version. This property should be used with the detect.project.tool property.")
    ...
    @HelpGroup(primary = GROUP_PATHS, additional = { GROUP_DETECTOR })
    @HelpDescription("The depth from source paths to search for files to determine if a BOM tool applies.")
    ...
    @HelpGroup(primary = GROUP_DETECTOR, additional = { GROUP_DETECTOR })
    @HelpDescription("If set, Detect fails if it does not find the BOM tool types supplied here.")
    ...
    @HelpGroup(primary = GROUP_GRADLE)
    @HelpDescription("The respository Gradle uses to search for the Gradle inspector dependencies.")
    ...
    @HelpDescription("The name of the NuGet Inspector package and the NuGet Inspector exe. Do not include .exe.")
    ...
    @HelpGroup(primary = GROUP_NUGET)
    @HelpDescription("The path of the NuGet executable. NuGet is used to download the classic inspector NuGet package.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("If set to true, the signature scanner, if supported by your Black Duck version, runs in snippet scanning mode.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("Only these paths are scanned.")
    ...
    @HelpDescription("To use a local signature scanner and force offline mode, specify the path where the signature scanner was unzipped. This resembles 'scan.cli-x.y.z,' and includes the 'bin, icon, jre, and lib' directories of the expanded scan.cli.")
    ...
    @HelpGroup(primary = GROUP_SIGNATURE_SCANNER, additional = { SEARCH_GROUP_HUB })
    @HelpDescription("If this URL is set, Detct attempts to use it to download the signature scanner. The provided server URL must respect the Black Duck URLs for different operating systems.")
    ...
    @HelpGroup(primary = GROUP_LOGGING, additional = { SEARCH_GROUP_GLOBAL })
    @HelpDescription("The Detect logging level.")
