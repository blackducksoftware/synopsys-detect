...
public class SignatureScannerDecisionBranch implements DecisionTree {
    ...
    public void traverse(InteractivePropertySourceBuilder propertySourceBuilder, InteractiveWriter writer) {
        if (connectedToBlackDuck) {
            ...
            if (!upload) {
                propertySourceBuilder.setProperty(DetectProperties.DETECT_BLACKDUCK_SIGNATURE_SCANNER_DRY_RUN, Boolean.TRUE.toString());
            }
        }

        Boolean customScanner = writer.askYesOrNo(SHOULD_USE_CUSTOM_SCANNER);
        if (customScanner) {
            propertySourceBuilder.setPropertyFromQuestion(DetectProperties.DETECT_BLACKDUCK_SIGNATURE_SCANNER_LOCAL_PATH, SET_SCANNER_OFFLINE_LOCAL_PATH);
        }
    }
}
