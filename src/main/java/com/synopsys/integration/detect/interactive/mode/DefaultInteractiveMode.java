...
public class DefaultInteractiveMode extends InteractiveMode {
    ...
    public void configure() {
        ...
        final Boolean connectToBlackduck = askYesOrNo("Would you like to connect to a Black Duck server?");
        if (connectToBlackduck) {
            attemptToConnectToBlackduck();
        } else {
            setProperty(DetectProperties.Companion.getBLACKDUCK_OFFLINE_MODE(), "true");
        }

        getSignatureScannerInfo(connectToBlackduck);
    ...
    private ConnectionResult testBlackduckConnection() {
        try {
            ...
            return blackDuckServerConfig.attemptConnection(new SilentIntLogger());
    ...
    private void getCertificateTrust() {
    ...
    private void attemptToConnectToBlackduck() {
        ...
        while (!connected && !skipConnectionTest) {
            ...
            final Boolean testBlackduckConnection = askYesOrNo("Would you like to test the Black Duck connection now?");
            if (testBlackduckConnection) {
                ConnectionResult connectionAttempt = testBlackduckConnection();
    ...
    private void getSignatureScannerInfo(boolean connectToBlackduck) {
        final Boolean runScanner = askYesOrNo("Would you like run a CLI scan?");
        if (!runScanner) {
            setProperty(DetectProperties.Companion.getDETECT_TOOLS_EXCLUDED(), "SIGNATURE_SCAN");
        } else if (connectToBlackduck) {
        ...
        getCustomScannerInfo(runScanner);
    }

    private void getCustomScannerInfo(final boolean runScanner) {
        if (runScanner) {
