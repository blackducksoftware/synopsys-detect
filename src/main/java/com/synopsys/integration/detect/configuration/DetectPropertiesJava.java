...
public class DetectPropertiesJava {
   ...
   static NullableStringProperty BLACKDUCK_API_TOKEN = new PropertyBuilder<>(new NullableStringProperty("blackduck.api.token")).build(property -> {
   ...
   static IntegerProperty BLACKDUCK_TIMEOUT = new PropertyBuilder<>(new IntegerProperty("blackduck.timeout", 120)).build(property -> {
    ...
    static StringListProperty  DETECT_BLACKDUCK_SIGNATURE_SCANNER_EXCLUSION_NAME_PATTERNS = new PropertyBuilder<>(new StringListProperty("detect.blackduck.signature.scanner.exclusion.name.patterns", singletonList("node_modules"))).build(property -> {
    ...
    static ExtendedEnumProperty  DETECT_BLACKDUCK_SIGNATURE_SCANNER_INDIVIDUAL_FILE_MATCHING = new PropertyBuilder<>(new ExtendedEnumProperty("detect.blackduck.signature.scanner.individual.file.matching", ExtendedEnumValue.ofExtendedValue(ExtendedIndividualFileMatchingMode.NONE), ExtendedIndividualFileMatchingMode.class, IndividualFileMatching.class)).build(property -> {
    ...
    static ExtendedEnumProperty  DETECT_BLACKDUCK_SIGNATURE_SCANNER_SNIPPET_MATCHING = new PropertyBuilder<>(new ExtendedEnumProperty("detect.blackduck.signature.scanner.snippet.matching", ExtendedEnumValue.ofExtendedValue(ExtendedSnippetMode.NONE), ExtendedSnippetMode.class, SnippetMatching.class)).build(property -> {
    ...
    static FilterableEnumListProperty  DETECT_POLICY_CHECK_FAIL_ON_SEVERITIES = new PropertyBuilder<>(new FilterableEnumListProperty("detect.policy.check.fail.on.severities", FilterableEnumUtils.noneList(), PolicyRuleSeverityType.class)).build(property -> {
    ...
    static EnumListProperty  DETECT_PROJECT_CLONE_CATEGORIES = new PropertyBuilder<>(new EnumListProperty("detect.project.clone.categories", Arrays.asList(ProjectCloneCategoriesType.COMPONENT_DATA, ProjectCloneCategoriesType.VULN_DATA), ProjectCloneCategoriesType.class)).build(property -> {
    ...
    static EnumProperty  DETECT_PROJECT_VERSION_DISTRIBUTION = new PropertyBuilder<>(new EnumProperty("detect.project.version.distribution", LicenseFamilyLicenseFamilyRiskRulesReleaseDistributionType.EXTERNAL, LicenseFamilyLicenseFamilyRiskRulesReleaseDistributionType.class)).build(property -> {
