...
public class YarnBomToolTest {
    ...
    public void testThatYarnLockIsParsedCorrectlyToMap() {
        ...
        yarnData.getYarnDataAsMap(lines);

        assertEquals("0.9.0", yarnData.getResolvedVersions().get("async@0.9.0"));
        assertEquals("1.0.3", yarnData.getResolvedVersions().get("colors@1.0.3"));
    }
    ...
    public void testThatYarnLockVersionsResolveAsExpected() {
        ...
        YarnDependencyData yarnData = new YarnDependencyData();
        yarnData.getYarnDataAsMap(lines);
        System.out.println(yarnData.getResolvedVersions());

        assertEquals("1.16.2", yarnData.getResolvedVersions().get("http-proxy@^1.8.1"));
        assertEquals("0.9.0", yarnData.getResolvedVersions().get("http-server@^0.9.0"));
    }
