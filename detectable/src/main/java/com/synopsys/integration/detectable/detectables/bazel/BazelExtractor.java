...
public class BazelExtractor {
    ...
    public Extraction extract(File bazelExe, File workspaceDir, BazelWorkspace bazelWorkspace, String bazelTarget,
        BazelProjectNameGenerator bazelProjectNameGenerator, List<WorkspaceRule> providedBazelDependencyTypes,
        List<String> providedCqueryAdditionalOptions) {
        logger.debug("Bazel extraction:");
        try {
            Set<WorkspaceRule> rulesFromWorkspaceFile = bazelWorkspace.getDependencyRules();
            ...
            String projectName = bazelProjectNameGenerator.generateFromBazelTarget(bazelTarget);
