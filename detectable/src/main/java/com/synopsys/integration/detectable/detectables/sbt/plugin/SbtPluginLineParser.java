...
public class SbtPluginLineParser {
    ...
    private boolean isAlphaNumericSpaceDashDot(String target) {
        return isAlphaNumericSpaceExtra(target, "-.");
    }
    ...
    private boolean isAlphaNumericSpaceExtra(String target, String extra) {
        if (target == null) {
            return false;
        }
        for (int i = 0; i < target.length(); i++) {
            ...
            for (int e = 0; e < extra.length(); e++) {
                isInExtra = isInExtra || extra.charAt(e) == c;
            }
            if (!isLetterOrDigit && !isSpace && !isInExtra) {
                return false;
            }
        }
        return true;
